class TeamMembersController < ApplicationController
  ...
  def ace
    @team_members = TeamMember.where(role: 'ACE Team Lead').or(TeamMember.where(role: 'ACE Developer')).or(TeamMember.where(role: 'ACE Designer'))
  end

  def crs
    @team_members = TeamMember.where(role: 'Class Representative')
  end
  ...
  private
    ...
    def team_member_params
      params.require(:team_member).permit(:name, :role, :email, :image)
    end
end
